...
bin_photos = mb.binarize(photo, bin_methods, resize=RESIZE,
                         morph_kernel=KERNEL, return_original=True)
...
for idx, bin_photo in enumerate(bin_photos[1:]):
    title = "Type: {}; w: {}; k: {}".format(
        bin_methods[idx]['type'], bin_methods[idx]['window_size'], bin_methods[idx]['k_factor'])
...
ut.plot_blobs(plot_photo, candidate_boxes, plot_line=True,
