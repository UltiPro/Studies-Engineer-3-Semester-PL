...
unsigned char read(void)
{
    ...
    if (bit_is_set2(PORT_1Wire, WE))
        PRESENCE = 1;
    else
        PRESENCE = 0; // odbior jedynki lub zera
...
int main(void)
{
    ...
    for (k = 0; k < 0x3600; k++)
        ;                           // czas na ustabilizowanie generatora napiecia odniesienia
    clearDisplay();                 // czyszczenie wyowietlacza
    clearDisplay();                 // czyszczenie wyowietlacza
    ...
    for (;;)
    {
        if (0 == 0)
        {
            ...
            if (sprawdz == 1)
            {
                ...
                for (k = 0; k < 9; k++)
                    __delay_cycles(600000);                 // odczekaj 750ms - czas konwersji
